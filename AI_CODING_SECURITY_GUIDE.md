...
## 0. AI 시키기 전 30초 체크 — 지금 당장

가장 흔한 대형 사고는 코드 로직이 아니라 **설정 실수**에서 납니다. AI를 부르기 전, 아래 3개부터 확인하세요.

### ① Git 저장소 공개 여부 확인

GitHub/GitLab에서 생성한 저장소가 **Public**인데 공개할 준비가 안 됐다면 지금 **Private으로 전환**하세요.

- GitHub: Repository → Settings → General → 맨 아래 "Change repository visibility" → Private
- GitLab: Settings → General → Visibility → Private

**왜 중요한가**: Public 레포는 검색엔진/봇이 실시간 감시합니다. `.env`를 실수로 커밋하면 **분 단위로** 공격자가 키를 줍습니다. AWS, Stripe, OpenAI 키는 실제로 30초 안에 탈취 사례가 나옵니다.

공개해도 되는 순간은 **"이 레포 전체를 지금 당장 HackerNews 1면에 올려도 문제없다"** 싶을 때입니다. 그 전엔 Private.

### ② `.gitignore`에 시크릿 파일 전부 포함
...
없으면 지금 추가하고 **기존에 커밋된 게 있는지** 확인:
```bash
git ls-files | grep -E "\\.env$|\\.key$|secret"
# 뭐라도 나오면: git rm --cached <파일> + 해당 키 전부 로테이션
```
...
발견되면:
1. **해당 키 즉시 로테이션** (새 키 발급 + 기존 키 폐기) — 가장 먼저
2. 그 다음에 [BFG Repo-Cleaner](https://rtyley.github.io/bfg-repo-cleaner/) 로 히스토리 정리
3. `git push --force` (협업자에게 사전 공지 필수)

**절대 하지 말 것**: "삭제 커밋 하나 추가"로 덮기 — 과거 커밋은 그대로 남아 여전히 조회 가능.
